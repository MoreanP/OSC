...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, dropout=None, hidden_dim=256, init_w=None, use_sample=False):
        ...
        # self.use_sample = 
        # if use_sample:
            # self.l3_log_std = nn.Linear(hidden_dim, action_dim)
...
class OSC_TD3(object):
    def __init__(
        ...
        self.osc_alpha = osc_alpha
        self.osc_epsilon = osc_epsilon
    ...
    def train(self, replay_buffer: ReplayBuffer, batch_size=256, logger=None):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # neg_log_beta = self.behavior.var_neglogprob(state, action)
            neg_log_beta = self.behavior.var_neglogprob(state, pi)

            # plt.hist(neg_log_beta.cpu().detach().numpy(), 100)
            # plt.savefig("diffusion1000_neg_log_beta_cheetah_m_r.png")
            # pdb.set_trace()
            ...
            actor_loss = - Q.mean() / Q.abs().mean().detach() - self.lambd * F.sigmoid((self.osc_epsilon-neg_log_beta)*self.osc_alpha).mean()
    ...
    def train_online(self, replay_buffer: ReplayBuffer, batch_size=256, logger=None):
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            if self.lambd_cool:
                lambd = self.lambd * max(self.lambd_end, (1.0 - self.total_it / self.max_timesteps))
                logger.log('train/lambd', lambd, self.total_it)
            else:
                lambd = self.lambd

            actor_loss = - Q.mean() / Q.abs().mean().detach() - lambd * F.sigmoid((self.osc_epsilon-neg_log_beta)*self.osc_alpha).mean()
